...
// Metric Statistics for OSH/KKP Notices
$total_notices = 0;
$active_notices = 0;
$draft_notices = 0;
...
if (isset($conn) && $conn) {
    // Total KKP notices
    $res_total = $conn->query("SELECT COUNT(*) FROM kkp_notices");
    if ($res_total) { list($total_notices) = $res_total->fetch_row(); }

    // Active notices
    $res_active = $conn->query("SELECT COUNT(*) FROM kkp_notices WHERE status = 'active' OR status = 'published'");
    if ($res_active) { list($active_notices) = $res_active->fetch_row(); }

    // Draft/Inactive notices
    $res_draft = $conn->query("SELECT COUNT(*) FROM kkp_notices WHERE status = 'draft' OR status = 'inactive'");
...
<head>
    ...
    <title><?php echo ($lang === 'en') ? 'OSH Announcements & Notices' : 'Pengurusan Notis & Pengumuman KKP'; ?> - BTMK</title>
        ...
        <div class="admin-hero">
            <div>
                <h1 style="margin: 0 0 0.4rem 0;">
                    <?php echo ($lang === 'en') ? 'OSH Notices & Announcements' : 'Pengurusan Notis & Pengumuman KKP'; ?>
                </h1>
                <p style="margin:0; opacity:0.9;">
                    <?php echo ($lang === 'en') ? 'Manage safety guidelines, health policies, and OSH unit announcements' : 'Cipta dan kemas kini panduan keselamatan, dasar kesihatan, serta pengumuman Unit KKP'; ?>
                </p>
            </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-info">
                    ...
                    <p><?php echo ($lang === 'en') ? 'Total OSH Notices' : 'Keseluruhan Notis KKP'; ?></p>
                </div>
        ...
        <div class="content-card">
            <div class="card-header">
                <div class="card-header-title">
                    ...
                    <h3><?php echo ($lang === 'en') ? 'All OSH Announcements' : 'Senarai Notis & Pengumuman KKP'; ?></h3>
                </div>
                <a href="admin_add_kkp_notice.php" class="btn-add">
                    ...
                    <span><?php echo ($lang === 'en') ? 'Create OSH Notice' : 'Tambah Notis KKP'; ?></span>
                </a>
            </div>
            ...
            <div class="table-responsive">
                <table class="custom-table">
                    ...
                    <tbody>
                        <?php
                        if (isset($conn) && $conn) {
                            $sql = "SELECT * FROM kkp_notices ORDER BY created_at DESC LIMIT 10";
                            ...
                            if ($result && $result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>
                                            <a href='admin_edit_kkp_notice.php?id=" . ($row['id'] ?? '') . "' style='color: var(--primary-blue); font-weight: bold; text-decoration: none; margin-right: 10px;'>Edit</a>
                                          </td>";
                                    echo "</tr>";
                                }
                            } else {
                                echo "<tr><td colspan='5' style='text-align: center; color: #777; padding: 20px;'>" . (($lang === 'en') ? 'No OSH notices found.' : 'Tiada rekod notis KKP dijumpai.') . "</td></tr>";
                            }
                        }
                        ?>
                    </tbody>
